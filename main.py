...
def load_data():
    ...
    if not os.path.exists(DATA_FILE):
        return [
            {"id": "1", "name": "Echo Dot", "quantity": "12", 'status': 'active'},
            {"id": "2", "name": "Kindle", "quantity": "4", 'status': 'active'},
        ]
...
def list_stock():
    ...
    for item in inventory:
        ...
        # LOGIC: Damaged items are Yellow, Low Stock is Red, else Green
        ...
        table.add_row(item["id"], item["name"], item["quantity"], item["status"], style=color)
    console.print(table)

@app.command()
def mark_damage(item_id: str):
    """Flags an item as damaged so it can be moved to Problem Solve."""
    inventory = load_data()
    found = False
    ...
    # MOVED OUT OF THE LOOP: Now it only runs ONCE after checking everything
    if found:
        save_data(inventory)
        console.print(f"[bold red]REPORTED:[/] Item {item_id} status updated to DAMAGED.")
    else:
        console.print(f"[bold yellow]Error:[/] ID {item_id} not found.")
